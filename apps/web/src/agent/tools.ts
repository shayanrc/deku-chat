// Browser port of server/tools.ts — identical tools, keys from the request only
// (a static page has no environment variables).
...
export function toolsFor(capabilities: string[], apiKeys: ApiKeys = {}) {
  const tools = [];
  if (capabilities.includes('web_search') && apiKeys.TAVILY_API_KEY) tools.push(makeWebSearch(apiKeys.TAVILY_API_KEY));
